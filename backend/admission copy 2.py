    ...
    try:
        ...
        if offer_info:
            ...
            cursor.execute("""
                ...
                LEFT JOIN semesters s ON tsr.semester_id = s.id
...
def get_all_admissions():
    ...
    try:
        ...
        base_query = """
            SELECT 
                tsr.id AS relation_id,
                COALESCE(sem.code, CAST(tsr.semester_id AS CHAR)) AS semester,
                tsr.created_at AS admitted_at,
                u_student.id AS student_id,
                u_student.name AS student_name,
                u_student.username AS student_number,
            ...
            FROM teacher_student_relations tsr
            LEFT JOIN semesters sem ON tsr.semester_id = sem.id
            JOIN users u_student ON tsr.student_id = u_student.id
            LEFT JOIN classes c ON u_student.class_id = c.id
        ...
        if semester:
            # 前端可能傳學期代碼（如 1132）或學期 id；先依 code 查 id，沒有則當 id 用
            semester_id_filter = None
            cursor.execute("SELECT id FROM semesters WHERE code = %s LIMIT 1", (str(semester).strip(),))
            row = cursor.fetchone()
            if row:
                semester_id_filter = row['id']
            else:
                try:
                    semester_id_filter = int(semester)
                except (ValueError, TypeError):
                    semester_id_filter = semester
            if semester_id_filter is not None:
                base_query += " AND tsr.semester_id = %s"
                params.append(semester_id_filter)
...
def vendor_matching_results():
    ...
    try:
        ...
        cursor.execute(f"""
            ...
            JOIN internship_companies ic ON sp.company_id = ic.id
            LEFT JOIN internship_jobs ij ON sp.job_id = ij.id
            LEFT JOIN teacher_student_relations tsr ON tsr.student_id = u.id AND tsr.semester_id = (SELECT id FROM semesters WHERE code = '1132' LIMIT 1)
            LEFT JOIN semesters sem ON tsr.semester_id = sem.id
